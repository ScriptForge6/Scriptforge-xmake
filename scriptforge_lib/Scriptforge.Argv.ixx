...
namespace Scriptforge {
    inline namespace Argv {
        ...
        ARGV_API()::ArgvCli(const int& argc, char* argv[], std::ostream& os, std::ostream& err, std::istream& is) {
            init(argc, argv, os, err, is);
        }

        ARGV_API(void)::init(const int& argc, char* argv[], std::ostream& os, std::ostream& err, std::istream& is) {
            m_os = &os;
            m_err = &err;
            m_is = &is;
        ...
        ARGV_API(const std::vector<std::string>&)::getArgv() const {
            return m_argv;
        }
    }
}